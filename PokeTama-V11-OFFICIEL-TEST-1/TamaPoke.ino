...
#include "home_gym_sprite.h"
#include "home_habitat_sprites.h"

// Version del firmware. Subir este numero en cada release (y manifest.json para
// el instalador web). Se muestra en la pantalla de ajustes y por serie al arrancar.
#define FW_VERSION "1.47.0-moretro3d-v11-official"
...
void setup() {
  ...
  // Version publique officielle V11.
  gfx->setTextColor(C565(0x61,0xda,0xf2));
  gfx->setTextSize(2);
  gfx->setCursor(CX - 18, 366); gfx->print("V11");
  gfx->flush();
...
void drawHomeGymCabinLegacy(uint8_t phase, uint8_t biome) {
...
// Affiche directement le sprite validé, sans le reconstruire avec des formes.
// Les pixels transparents laissent apparaître le décor dynamique de l'accueil.
void drawHomeGymCabin(uint8_t phase, uint8_t biome) {
  ...
  for (uint8_t y=0; y<HOME_GYM_H; y++) {
    uint8_t x=0;
    while (x<HOME_GYM_W) {
      uint8_t color=pgm_read_byte(&HOME_GYM_PIXELS[(uint16_t)y*HOME_GYM_W+x]);
      uint8_t start=x++;
      while (x<HOME_GYM_W &&
             pgm_read_byte(&HOME_GYM_PIXELS[(uint16_t)y*HOME_GYM_W+x])==color) x++;
      if (color) {
        uint16_t rgb=pgm_read_word(&HOME_GYM_PALETTE[color]);
        gfx->fillRect(x0+start,y0+y,x-start,1,rgb);
      }
    }
  }
}

// Sols d'accueil validés : six décors pixel art partageant exactement la
// même ligne d'horizon. Chaque pixel source est affiché en bloc 2x2.
void drawHomeHabitatGround(uint8_t biome) {
  uint8_t b=biome<6?biome:0;
  const uint8_t *pixels=HOME_HABITAT_PIXELS[b];
  const uint16_t *palette=HOME_HABITAT_PALETTES[b];
  const int16_t x0=-17;
  // Volcan reste à sa position dédiée ; Roche est remonté exactement de 4 px.
  const int16_t y0=(b==3)?181:(b==4?189:193);
  for (uint8_t y=0; y<HOME_HABITAT_H; y++) {
    uint8_t x=0;
    while (x<HOME_HABITAT_W) {
      uint8_t color=pgm_read_byte(&pixels[(uint16_t)y*HOME_HABITAT_W+x]);
      uint8_t start=x++;
      while (x<HOME_HABITAT_W &&
             pgm_read_byte(&pixels[(uint16_t)y*HOME_HABITAT_W+x])==color) x++;
      if (color) {
        uint16_t rgb=pgm_read_word(&palette[color]);
        gfx->fillRect(x0+start*2,y0+y*2,(x-start)*2,2,rgb);
      }
    }
  }
}
...
void drawScene(uint8_t biome, uint32_t now, bool night) {
  ...
  uint8_t b=biome<6?biome:0;
  // Aucun ancien habitat n'est dessine ici : ni bande d'eau, ni arbres, ni
...
void closeBattle() {
  bool returnToArena = battleArena;
  battleOpen = false;
...
void finishBattle() {
  ...
  if (battleTurn.playerWon) {
    ...
    battleCatchOffered = !battleArena;
    battleRespectCatch = false;
    battleCatchChance = pet.catchChanceForWild(battleDex, battleLevel, battlePlayer.level, closeWin);
    if (battleArena) {
      pet.awardKantoBadge(battleArenaIndex);
      battleArenaBadgeWon = true;
      battleCatchChance = 0;
    }
    ...
    battleCatchOffered = !battleArena && battleCatchChance > 0;
...
uint8_t battleGroundStyleFor(uint8_t type1,uint8_t type2) {
  // Priorité stable pour les doubles types : l'élément du Pokémon garde son
  // décor (Dracaufeu=Feu, Léviator=Eau, Artikodin=Glace), puis Vol utilise
  // les nuages quand aucun élément de décor plus précis n'est présent.
  if(type1==TYPE_ICE||type2==TYPE_ICE) return 5;
  if(type1==TYPE_WATER||type2==TYPE_WATER) return 1;
  if(type1==TYPE_FIRE||type2==TYPE_FIRE) return 2;
  if(type1==TYPE_STEEL||type2==TYPE_STEEL) return 4;
  if(type1==TYPE_GROUND||type2==TYPE_GROUND) return 4;
  if(type1==TYPE_ROCK||type2==TYPE_ROCK) return 4;
  if(type1==TYPE_GRASS||type2==TYPE_GRASS||type1==TYPE_BUG||type2==TYPE_BUG) return 0;
  if(type1==TYPE_FLYING||type2==TYPE_FLYING) return 6;
  return 3;
}

uint8_t battleGroundStyleForDex(int16_t dex) {
  if(dex<1 || dex>DEX_COUNT) return 3;
  return pgm_read_byte(&BATTLE_GROUND_BY_DEX[dex]);
}
...
void drawBattleBaseFor(uint8_t style,bool enemy) {
  ...
  switch(style) {
    ...
    case 6: pal=enemy?BATTLE_BASE_E_6_PAL:BATTLE_BASE_P_6_PAL; rle=enemy?BATTLE_BASE_E_6_RLE:BATTLE_BASE_P_6_RLE; rleSize=enemy?sizeof(BATTLE_BASE_E_6_RLE):sizeof(BATTLE_BASE_P_6_RLE); break;
  }
...
void renderBattle() {
  ...
  drawBattleBaseFor(battleGroundStyleForDex(battleDex),true);
  drawBattleBaseFor(battleGroundStyleForDex(playerDex),false);
  ...
  if (battleResolved) {
    ...
    int infoY = 373;
    if (battleArena && battleTurn.playerWon && battleArenaBadgeWon) {
      static const char *const WON[LANG_COUNT]={"INSIGNIA GANADA","BADGE EARNED","BADGE OBTENU","ORDEN ERHALTEN","MEDAGLIA OTTENUTA","INSIGNIA OBTIDA"};
      const char *won=WON[gLang<LANG_COUNT?gLang:LANG_EN];
      gfx->setTextColor(UI_BAR_WARN);
      gfx->setTextSize(2);
      gfx->setCursor(CX-(int)strlen(won)*6,infoY);
      gfx->print(won);
...
static const char *const KANTO_LEADER_NAMES[LANG_COUNT][8] = {
  { "BROCK", "MISTY", "LT. SURGE", "ERIKA", "KOGA", "SABRINA", "BLAINE", "GIOVANNI" },
  { "BROCK", "MISTY", "LT. SURGE", "ERIKA", "KOGA", "SABRINA", "BLAINE", "GIOVANNI" },
  { "PIERRE", "ONDINE", "MAJOR BOB", "ERIKA", "KOGA", "MORGANE", "AUGUSTE", "GIOVANNI" },
  { "ROCKO", "MISTY", "MAJOR BOB", "ERIKA", "KOGA", "SABRINA", "PYRO", "GIOVANNI" },
  { "BROCK", "MISTY", "LT. SURGE", "ERIKA", "KOGA", "SABRINA", "BLAINE", "GIOVANNI" },
  { "BROCK", "MISTY", "LT. SURGE", "ERIKA", "KOGA", "SABRINA", "BLAINE", "GIOVANNI" }
};

const char *kantoLeaderName(uint8_t index) {
  uint8_t lang = gLang < LANG_COUNT ? (uint8_t)gLang : (uint8_t)LANG_EN;
  return index < 8 ? KANTO_LEADER_NAMES[lang][index] : "?";
}

const char *kantoArenaTitle() {
  static const char *const L[LANG_COUNT] = { "LIGAS KANTO", "KANTO GYMS", "ARENES KANTO", "KANTO-ARENEN", "PALESTRE KANTO", "GINASIOS KANTO" };
  return L[gLang < LANG_COUNT ? gLang : LANG_EN];
}

const char *kantoFightText() {
  static const char *const L[LANG_COUNT] = { "COMBATIR?", "BATTLE?", "COMBATTRE ?", "KAMPFEN?", "LOTTARE?", "LUTAR?" };
  return L[gLang < LANG_COUNT ? gLang : LANG_EN];
}
...
uint8_t kantoBadgeCount() {
  uint8_t n=0;
  for(uint8_t i=0;i<8;i++) if(pet.hasKantoBadge(i)) n++;
  return n;
}
...
void renderCard() {
  ...
  else if (cardPage == 8) renderCardRecords();
  else renderCardKantoGyms();
