...
ok("1.47.0-moretro3d-v11-official" in ino and
   "1.47.0-moretro3d-v11-official" in (ROOT/"web/manifest.json").read_text(encoding="utf-8") and
   "1.47.0-moretro3d-v11-official" in (ROOT/"web/index.html").read_text(encoding="utf-8") and
   "1.47.0-moretro3d-v11-official" in (ROOT/"web/shopify.html").read_text(encoding="utf-8") and
   "1.47.0-moretro3d-v11-official" in (ROOT/"tools/build_web.sh").read_text(encoding="utf-8"),
   "version 1.47.0 coherente et caches installateur invalides")
ok('gfx->setCursor(CX - 18, 366); gfx->print("V11")' in ino and
   '<div class="version">Firmware V11</div>' in (ROOT/"web/index.html").read_text(encoding="utf-8") and
   '<div class="version">Firmware V11</div>' in (ROOT/"web/shopify.html").read_text(encoding="utf-8"),
   "V11 affichee au demarrage, sur la page officielle et sur Shopify")
...
ok("https://moretro3d.github.io/Test-2/?v=1.47.0-moretro3d-v11-official" in test2_launcher,
...
ok("HORIZON-38" not in scene and "uint16_t sea=" not in scene and
   "Aucun ancien habitat n'est dessine ici" in scene and
   "drawHomeGymCabin(phase,b);" in scene and "drawHomeHabitatGround(b);" in scene,
   "ancienne bande d'eau supprimee, ciel dynamique et nouveaux sols conserves")
...
ok("if(type1==TYPE_FIRE||type2==TYPE_FIRE) return 2" in ino and
   "if(type1==TYPE_STEEL||type2==TYPE_STEEL) return 4" in ino and
   "if(type1==TYPE_GROUND||type2==TYPE_GROUND) return 4" in ino and
   "if(type1==TYPE_FLYING||type2==TYPE_FLYING) return 6" in ino and
   "return 3;" in ino,
   "priorites des sept familles de sols explicites")
...
ok("void drawBoxSelectionPmd(PmdMon &m, int cx, int groundY)" in ino and
   "int drawH=max(1,visibleH*drawW/visibleW);" in ino and
   "if(galleryPmd.loaded) drawBoxSelectionPmd(galleryPmd,CX,244);" in ino and
   "galleryPmd.load(dex,pet.isShinyRegistered(dex));" in ino and
   "if(thumb) drawBoxSelectionThumbCentered(thumb,CX,194);" in ino and
   "if (thumb) drawStarterThumbCentered(thumb, dex, x + 32, y + 31, 2);" in ino,
   "sprite PMD complet du popup proportionnel avec miniature de secours")
ok("gfx->fillRoundRect(82,302,144,44,12,UI_TRACK);" in ino and
   "gfx->fillRoundRect(240,302,144,44,12,UI_BAR_OK);" in ino and
   "x>=82 && x<=226 && y>=302 && y<=346" in ino and
   "x>=240 && x<=384 && y>=302 && y<=346" in ino,
   "boutons Retour et S'occuper de taille et zones tactiles identiques")
...
ok("void drawHomeGymCabin(uint8_t phase, uint8_t biome)" in ino and
   ...
   "x >= 155 && x <= 311 && y >= 122 && y <= 270" in ino and
   "const int16_t x0=155, y0=128" in ino and
...
ok('#include "home_habitat_sprites.h"' in ino and
   ...
   "const int16_t x0=-17" in ino and "const int16_t y0=(b==3)?181:(b==4?189:193)" in ino and
...
ok((ROOT/"assets/home_habitat_mountain_v10_31.png").exists() and
...
ok(all((ROOT/"assets"/f"battle_ground_{name}_v10_32.png").exists()
       for name in ("grass","water","fire","normal","rock","ice","cloud")),
   "sept nouveaux pixel arts joueur et adversaire integres au combat")
...
ok('#include "battle_bases.h"' in ino and
   '#include "battle_ground_map.h"' in ino and
   "drawBattleBaseFor(battleGroundStyleForDex(battleDex),true)" in ino and
   "drawBattleBaseFor(battleGroundStyleForDex(playerDex),false)" in ino and
   "gfx->fillRect(-23+sx*2,102+sy*2,take*2,2,color)" in ino,
   "sol de chacun des 386 Pokemon choisi par table auditee")
ok("ENEMY_BASELINE = 48" in (ROOT/"tools/make_battle_bases.py").read_text(encoding="utf-8") and
   "PLAYER_BASELINE = 108" in (ROOT/"tools/make_battle_bases.py").read_text(encoding="utf-8") and
   ...
   "sept demi-sols joueur a Y108 et adversaires entiers a Y48")
ground_generator=(ROOT/"tools/make_battle_bases.py").read_text(encoding="utf-8")
ok(all(f'(\"{name}\",' in ground_generator for name in
       ("PLANTE","EAU","FEU","NORMAL","ROCHE","GLACE","NUAGES")) and
   (ROOT/"tools/audit_battle_grounds.py").exists() and
   (ROOT/"tools/generate_battle_ground_map.py").exists() and
   len((ROOT/"assets/battle-ground-map-386.csv").read_text(encoding="utf-8").splitlines())==387,
   "sept sols semantiques et affectation des 386 Pokemon audites")
