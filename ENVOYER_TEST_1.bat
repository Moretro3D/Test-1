...
title PokeTama V10 - Deploiement officiel Test-1
cd /d "%~dp0"
...
echo Preparation de la version officielle V10...
git -c safe.directory="%CD%" add -A
git -c safe.directory="%CD%" diff --cached --quiet
if errorlevel 1 (
  git -c safe.directory="%CD%" commit -m "PokeTama V10 - version officielle"
  if errorlevel 1 goto :connexion
)
...
if errorlevel 1 (
  git -c safe.directory="%CD%" merge -s ours --no-edit origin/main -m "Rattache PokeTama V10 a Test-1"
  if errorlevel 1 goto :connexion
)
